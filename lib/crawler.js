...
class Crawler {
  ...
  async _collectLinks(baseUrl) {
    const links = [];
    const className = this._options.parentClassName ? '.' + this._options.parentClassName+ ' ': '';
    ...
    await this._page.evaluate(() => {
      function findLinks(document) {
        document.querySelectorAll(className + 'a[href]')
          .forEach(link => {
            // @ts-ignore
            window.pushToLinks(link.href);
          });
        document.querySelectorAll(className + 'iframe,frame')
          .forEach(frame => {
            try {
