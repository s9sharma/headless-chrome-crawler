...
class Crawler {
  ...
  async _collectLinks(baseUrl) {
    const links = [];
    const classNames =  this._options.parentClassNames ? this._options.parentClassNames : [];
    const querySelectors = classNames.map( (className, index) => '.' + className + ' a[href]' +  (index === classNames.length -1 ? '' : ','));

    await this._page.exposeFunction('pushToLinks', link => {
      const _link = resolveUrl(link, baseUrl);
      if (_link) links.push(_link);
    });
    await this._page.evaluate((querySelectors, includeIFrames) => {
      function findLinks(document) {
        document.querySelectorAll(querySelectors)
          .forEach(link => {
            // @ts-ignore
    ...
    }, querySelectors, this._options.includeIFrames);
    return uniq(links);
  }
  /**
